# Sylva Platform Core — 使用说明书

> **核心定位**：Ollama 本地子代理 + 多 Provider 统一编排引擎 + Hermes 内嵌记忆 + 蜂群协调  
> **设计目标**：发给普通 OpenClaw，解压 → 改 env → 启动 → 直接用  
> **版本**：v1.1.0 | 2026-06-01

---

## 这是什么？

这是我们从完整版 Sylva Platform 中提取的**核心引擎 v1.1**，包含：
...
```
╔══════════════════════════════════════════════╗
║   Sylva Platform Core v1.1.0                  ║
║   Ollama子代理 + 多Provider编排 + Hermes记忆    ║
╠══════════════════════════════════════════════╣
║   🦙 Ollama桥接:  http://localhost:3001/api/ollama    ║
║   🔀 统一API:     http://localhost:3001/api/unified   ║
║   🎼 编排API:     http://localhost:3001/api/unified/orchestrate ║
...
cp -r sylva_platform_core ~/.openclaw/skills/sylva-orchestrator/
```
...
Sylva Core 本身就是一个 Express 服务器，OpenClaw 通过 HTTP 调用它：
...
然后 OpenClaw 的 tool call 可以直接调 Sylva 的 API。
...
| 版本 | 日期 | 变更 |
|------|------|------|
| v1.0.0 | 2026-06-01 | 初始提取：Ollama桥接 + 10 Provider + Claude编排 + Hermes兼容 |
| v1.1.0 | 2026-06-01 | 新增：完整后端引擎（5,773文件）+ Hermes内嵌记忆 + 蜂群协调 + 37份架构文档 |
...
— Sylva Platform Core v1.1.0, extracted for sharing.

---

## 反馈与扩展

如果你基于这个包做了扩展，或者发现 Bug，欢迎记录到 `memory/` 目录下的日记中。这个包的设计意图就是**可移植、可复用、可组合**——把它当成你 OpenClaw 工具箱里的一件瑞士军刀。